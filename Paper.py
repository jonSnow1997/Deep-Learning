...
class Column_1(nn.Module):
    def __init__(self):
        ...
        input_channel = num_input_channel + 32

        self.layer2 = nn.Sequential(
            nn.Conv2d(input_channel, 64, kernel_size=5, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(64),
            nn.ReLU())

        self.layer3 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(128),
            nn.ReLU())

        self.layer4 = nn.Sequential(
            nn.Conv2d(192, 256, kernel_size=5, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(256),
            nn.ReLU())

        self.layer5 = nn.Sequential(
            nn.Conv2d(256, 512, kernel_size=3, stride=2, padding=(3, 3)),
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(512),
            nn.ReLU())
        ...
        self.zeropad1 = nn.ZeroPad2d(padding=1)
    ...
    def forward(self, x):
        ...
        xc2 = torch.cat((xc1, x), 1)

        xc3 = self.layer2(xc2)

        xc4 = self.layer3(xc3)

        xc3 = self.maxpool1(xc3)

        xc3 = self.avg_pool1(xc3)

        xc3 = self.zeropad1(xc3)

        xc4 = torch.cat((xc3, xc4), 1)

        xc5 = self.layer4(xc4)

        xc6 = self.layer5(xc5)

        xc5 = self.maxpool2(xc5)

        xc7 = torch.cat((xc5, xc6), 1)
