...
class Column_1(nn.Module):
    def __init__(self):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(num_input_channel, 32, kernel_size=7, stride=2, padding=(3, 3)),
            nn.BatchNorm2d(32),
            nn.ReLU())

        input_channel = num_input_channel + 128

        self.layer2 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=5, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(64),
            nn.ReLU())

        self.layer3 = nn.Sequential(
            nn.Conv2d(input_channel, 128, kernel_size=3, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(128),
            nn.ReLU())

        self.layer4 = nn.Sequential(
            nn.Conv2d(input_channel, 256, kernel_size=5, stride=2, padding=(3, 3)),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(256),
            nn.ReLU())

        self.layer5 = nn.Sequential(
            nn.Conv2d(288, 512, kernel_size=3, stride=2, padding=(3, 3)),
            # nn.MaxPool2d(kernel_size=2, stride=2),
            nn.BatchNorm2d(512),
            nn.ReLU())
    ...
    def forward(self, x):
       ...
       xc4 = torch.cat((x, xc3), 1)

       xc5 = self.layer4(xc4)

       xc1 = self.maxpool3(xc1)

       xc1 = self.avg_pool3(xc1)

       xc6 = torch.cat((xc1, xc5), 1)

       xc7 = self.layer5(xc6)

       out = xc7.view(-1, self.num_flat_features(xc7))
